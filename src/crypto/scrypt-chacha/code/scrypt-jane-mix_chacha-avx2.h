/* AVX2 ChaCha20/8 mixer for scrypt-jane.
 *
 * Functionally this tier emits the same VEX.128-encoded ChaCha instruction
 * stream as the AVX variant — the canonical AVX2 "two-blocks-in-parallel"
 * 256-bit rewrite does not apply to YAC's r = 1 case, because the two
 * blocks of a chunk-mix iteration are sequentially dependent on each
 * other and cannot be ChaCha-rounded in parallel.
 *
 * What this file unlocks is a distinct dispatch tier gated on the cpuid
 * AVX2 bit. The runtime mixer-selection in scrypt-jane-chacha.h reports
 * "AVX2" on AVX2-capable hardware and "AVX" otherwise, so the dispatcher
 * stays honest about what the CPU actually advertises. If a genuinely
 * faster scrypt-chacha mixer for r = 1 is found later (e.g. via VPGATHER
 * or AVX-512 BW lane permutes), it can be slotted in here without
 * touching the runtime plumbing. */

/* x86 */
#if defined(X86ASM_AVX2) && (!defined(SCRYPT_CHOOSE_COMPILETIME) || !defined(SCRYPT_CHACHA_INCLUDED)) && !defined(CPU_X86_FORCE_INTRINSICS)

#define SCRYPT_CHACHA_AVX2

asm_naked_fn_proto(void, scrypt_ChunkMix_avx2)(uint32_t *Bout/*[chunkBytes]*/, uint32_t *Bin/*[chunkBytes]*/, uint32_t *Bxor/*[chunkBytes]*/, uint32_t r)
...
/* x64 */
#if defined(X86_64ASM_AVX2) && (!defined(SCRYPT_CHOOSE_COMPILETIME) || !defined(SCRYPT_CHACHA_INCLUDED)) && !defined(CPU_X86_FORCE_INTRINSICS)
...
/* intrinsic */
#if defined(X86_INTRINSIC_AVX2) && (!defined(SCRYPT_CHOOSE_COMPILETIME) || !defined(SCRYPT_CHACHA_INCLUDED))

#define SCRYPT_CHACHA_AVX2

static void asm_calling_convention NOINLINE
scrypt_ChunkMix_avx2(uint32_t *Bout/*[chunkBytes]*/, uint32_t *Bin/*[chunkBytes]*/, uint32_t *Bxor/*[chunkBytes]*/, uint32_t r) {
...
#if defined(SCRYPT_CHACHA_AVX2)
	#undef SCRYPT_MIX
	#define SCRYPT_MIX "ChaCha/8-AVX2"
